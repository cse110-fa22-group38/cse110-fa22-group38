...
async function getCoursesAndIcs(myCoursesPar, icsPar) {
    // Grabbing data of all current courses
    myCoursesPar = await getCurrentCourses();

    // Grabbing raw texts from the ICS file of all my current courses
    icsPar = await getICStexts(myCourses);
}

getCoursesAndIcs(myCourses, icsStringArray);
